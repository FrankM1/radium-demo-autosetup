<ul class="order">
   <li><?php _e('Order', 'unsplash_plugin'); ?>:</li>
   <li><a href="javascript:void(0);" id="usp-newest"><?php _e('Recent', 'unsplash_plugin'); ?></a></li>
   <li><a href="javascript:void(0);" id="usp-oldest"><?php _e('Oldest', 'unsplash_plugin'); ?></a></li>
   <li><a href="javascript:void(0);" id="usp-randomize"><?php _e('Random', 'unsplash_plugin'); ?></a></li>
</ul>
<ul id="usp_photos"></ul>
<div class="more-wrap">
   <span class="loading"></span>
   <input type="button" id="usp-load-more" name="usp-load-more" value="<?php _e('Load More Images', 'unsplash_plugin'); ?>" class="button">
</div>
<?php
   ...
   jQuery(document).ready(function($) {
      // Parse JSON from https://unsplash.it/list
       ...
       $.getJSON( url, function( data ) {
         ...
         console.log("<?php _e('Error loading Unsplash JSON feed from https://unsplash.it/list.', 'unsplash_plugin'); ?>")
      });
           ...
           if (undefined != usp_photos[n]){
            //console.log(usp_photos[n]);
              var id = usp_photos[n].id,
                  ...
                  el = '<li>';
                  el += '<a class="upload" href="'+ link +'" data-url="'+full_size+'" data-desc="<?php _e('Photo by: ', 'unsplash_plugin'); ?> '+author+'" title="<?php _e('Click to upload photo ', 'unsplash_plugin'); ?>">';
                  el += '<img width="'+ usp_thumb_width +'" height="'+ usp_thumb_height +'" src="'+ img +'"/>'
                  el += '</a>';
                  el += '<div class="controls">';
                  el += '<span class="num zoom-in" data-href="'+link+'" title="<?php _e('View Full Size', 'unsplash_plugin'); ?>"><i class="fa fa-search-plus"></i></span>';
               ...
               $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if ( response ){
                                ...
                                el.removeClass('saving').removeClass('uploaded');
                                if(!$('span.err', el).length){
                                   el.append('<span class="err" title="<?php _e('Unable to save image, check your server permissions.', 'unsplash_plugin');?>"><i class="fa fa-exclamation-circle"></i></span>');
                                }
                             }
